...
vaultPoints = doc.vaultPoints + lastDebtValue × (vaultDays)^1.7 × 0.002

**Variables:**

- `doc.vaultPoints`: previously accumulated points  
- `lastDebtValue`: your vault debt in SUI (1 SUI = 1e9 MIST)
...
stakingPoints = doc.stakingPoints + lastStakingValue × (stakingDays)^1.4 × 1.2 × 0.005


**Variables:**

- `doc.stakingPoints`: previously accumulated staking points  
- `lastStakingValue`: amount staked in SUI (1 SUI = 1e9 MIST)
...
flowXPoints += liquidity × (days)^2.0 × multiplier × 0.015

Parameters:
- **liquidity** – amount of liquidity provided in MIST
